...
class GameView:
    def __init__(self, game):
        ...
        #Stats would be a listbox
        deck_column = [
            [sg.Text('Deck')],
            [sg.Listbox(values=Board.Board().deck, size=(30, 10))]
        ]
        bank_column = [
            [sg.Text('Bank')],
            [sg.Listbox(values=Bank.Bank().resources.items(), size=(30, 10))],
            [sg.Text("Dice: " + str(self.game.dices[self.game.actualTurn]))]

        ]
        graph_column = [
            [sg.Graph((600, 400), (0, 0), (500, 500), background_color='lightblue', key='GRAPH')],
        ]
        stats_column = [
            [sg.Text('Moves list')],
            [sg.Listbox(values=[], size=(30, 10), key='moves_list')]
        ]

        layout = [
                    [sg.Text('CatanAI test')],
                    [sg.Text('GNN Scores', border_width=4, key='GNN_vals')],                    
                    [sg.Column(deck_column),
                    sg.Column(graph_column),
                    sg.Column(bank_column),
                    sg.Column(stats_column)],
                    [sg.Button('AI Move'), sg.Button('Random Move'), sg.Button('UNDO', button_color='firebrick'), sg.Button('REDO'), sg.Exit()]
                ]
        self.sgWindow = sg.Window('Test window', layout, element_justification='c', finalize=True)
    ...
    def updateScoreGNN(self, player):
        ...
        self.sgWindow['GNN_vals'].update(scores_string)        
    ...
    def updateGameScreen(self):
        ...
        self.sgWindow['moves_list'].update(ctr.undoStack)
