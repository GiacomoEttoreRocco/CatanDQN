...
class GameView:
    ...
    def displayInitialBoard(self):
        ...
        resColorDict_RGB = {"clay": (255, 51, 51), "iron": (128, 128, 128), "crop": (255, 255, 51), "wood": (0, 153, 0),
                         "sheep": (51, 255, 51), "desert": (255, 255, 204)}
        pygame.draw.rect(self.screen, pygame.Color('white'),
                         (0, 0, 1000, 800))  # blue background

        # Render each hexTile
        pointy = Layout(layout_pointy, Point(80, 80),
        ...
        for boardtile in Board().tiles:
            ...
            graphicTileCorners = polygon_corners(pointy, graphicTile.hex)
            tileColorRGB = resColorDict_RGB[boardtile.resource]
            pygame.draw.polygon(self.screen, pygame.Color(tileColorRGB[0], tileColorRGB[1], tileColorRGB[2]), graphicTileCorners, width == 0)
            graphicTile.pixelCenter = hex_to_pixel(pointy, graphicTile.hex)
    ...
    def setupInitialPlaces(self):
        ...
        for place in Board().places:
            self.graphicPlaceList.append(GraphicPlace(place))
        alreadyFound = []
        for gtile in self.graphicTileList:
            for k, v in tilePlaces.items():
                if gtile.index == k:
                    for el in v:
        ...
        # for gtile in self.graphicTileList:
        #     gtile.places.append(GraphicPlace(Board().places[step], ((gtile.pixelCenter.x - 70), (gtile.pixelCenter.y - 40))))
        #     gtile.places.append(GraphicPlace(Board().places[step], ((gtile.pixelCenter.x + 70), (gtile.pixelCenter.y + 40))))
        #     gtile.places.append(GraphicPlace(Board().places[step], ((gtile.pixelCenter.x - 70), (gtile.pixelCenter.y + 40))))
        #     gtile.places.append(GraphicPlace(Board().places[step], ((gtile.pixelCenter.x + 70), (gtile.pixelCenter.y - 40))))
        #     gtile.places.append(GraphicPlace(Board().places[step], (gtile.pixelCenter.x, (gtile.pixelCenter.y + 75))))
        #     gtile.places.append(GraphicPlace(Board().places[step], (gtile.pixelCenter.x, (gtile.pixelCenter.y - 75))))
    ...
    def drawPlace(self, graphicPlace):
        ...
        if graphicPlace.harbor is not None:
            harborText = self.font_harbors.rfrom GraphicTile import *ender(graphicPlace.harbor, False, (0, 0, 0))
